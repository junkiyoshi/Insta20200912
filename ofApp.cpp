...
void ofApp::update() {
	...
	int radius = 50;
	for (int deg = 0; deg < 360 * 5; deg += 2) {

		this->setRingToMesh(this->face, this->frame, glm::vec3(), radius, radius * 0.2, 16, deg, deg + 1);
		radius += 1;
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, int deg_start, int deg_end) {
	...
	for (int deg = deg_start; deg <= deg_end; deg += 1) {
		...
		auto noise_value_1 = ofNoise(deg * 0.01 + ofGetFrameNum() * 0.01 * -1);
		auto z = ofMap(noise_value_1, 0, 1, -200, 200);

		auto next_noise_value = ofNoise((deg + 1) * 0.01 + ofGetFrameNum() * 0.01 * -1);
		auto next_z = ofMap(next_noise_value, 0, 1, -200, 200);
